...
def play(strategy0, strategy1, update,
    ...
    if update == simple_update:
        while score0 < goal and score1 < goal:
            ...
            who = abs(1-who)
    elif update == square_update:
        while score0 < goal and score1 < goal:
            if who == 0:
                num_rolls = strategy0(score0, score1)
                score0 += take_turn(num_rolls, score1, dice)
                if who == 0 and perfect_square(score0) == True:
                    score0 = next_perfect_square(score0)
            elif who == 1:
                num_rolls = strategy1(score1, score0)
                score1 += take_turn(num_rolls, score0, dice)
                if who == 1 and perfect_square(score1) == True:
                    score1 = next_perfect_square(score1)
            who = abs(1-who)        
    # END PROBLEM 5
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while roll_num <= 10:
        ...
        max_for_roll_num = average_roll(roll_num, dice)
        if max_for_roll_num > max:
            max = max_for_roll_num
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    if tail_points(opponent_score)>= threshold:
        return 0
    elif perfect_square(tail_points(opponent_score)) == True:
        new_score = next_perfect_square(tail_points)
    ...
    ##if tail_strategy(score, opponent_score, threshold, num_rolls) == 0:
        #new_score = square_update(num_rolls, score, opponent_score, dice=six_sided)
    #else:
      #  new_score = simple_update(num_rolls, score, opponent_score, dice=six_sided)
    #if new_score - score >= threshold:
    #    return 0
    #else:
